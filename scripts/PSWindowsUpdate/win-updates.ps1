...
Start-Sleep -s 30
###Kill OOBE Popup
#Get-Process "sysprep" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue | Out-Null

###Install MS Store Updates
Function Install-MSStoreUpdates {
  ...
  switch ($global:RestartRequired) {
    0 {
      ...
      elseif ($script:Cycles -gt $global:MaxCycles) {
        ...
        New-Item C:\Windows\Temp\TokenFile.txt -Force | Out-Null
        Write-Host -ForegroundColor Green "Token File created, starting 5 minute countdown until Sysprep..."
        $UserClean = Get-LocalUser -Name "BuildAdmin" -ErrorAction SilentlyContinue
        if ($UserClean) {
          Write-Host -ForegroundColor Yellow "BuildAdmin account detected.  Will be removed after Sysprep countdown"
        }
        Write-Host -ForegroundColor Yellow "Triggering MS Store Updates..."
        $LC = 1
        do {
          Write-Host -ForegroundColor Yellow "Pass ($($LC))..."
          Install-MSStoreUpdates
          Start-Sleep -s 180
          $LC++
        }
        until ($LC -eq 5)
        ...
        if ($UserClean) {
          Get-LocalUser -Name "BuildAdmin" -ErrorAction SilentlyContinue | Remove-LocalUser -Confirm:$false -ErrorAction Stop
        }

        Write-Host -ForegroundColor Yellow "Sysprep in 5 minutes..."
        Start-Sleep -s 300
        ###Clean Run Scripts
        Write-Host -ForegroundColor Yellow "Cleaning Run Keys..."
        $HKLMPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
        $RunScripts = (Get-Item -Path $HKLMPath).Property | ? {$_ -like "Unattend*"}
        $RunScripts | ForEach {
            Remove-ItemProperty -Path $HKLMPath -Name $_ -Force -ErrorAction Stop
        }
        ###Kill OOBE Popup
        Get-Process "sysprep" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue | Out-Null
        Start-Sleep -s 5
        ...
        #& "C:\Windows\Installer\PSWindowsUpdate\enable-winrm.ps1"
    ...
    1 {
      ...
      Start-Process -FilePath "C:\Windows\System32\Sysprep\Sysprep.exe" -ArgumentList "/audit /reboot"
      #Restart-Computer
...
function Check-WindowsUpdates() {
  ...
  while (-not $script:successful -and $script:attempts -lt $script:maxAttempts) {
    try {
      $script:SearchResult = $script:UpdateSearcher.Search("IsInstalled=0 and Type='Software' and IsHidden=0 and IsAssigned=1")
