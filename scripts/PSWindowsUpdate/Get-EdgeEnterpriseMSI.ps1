...
$FinalDest = ($WinTemp + "\" + "MicrosoftEdgeEnterprisex64.msi")
...
$CurrentEdge = (Get-Item -Path ${env:ProgramFiles(x86)}\Microsoft\Edge\Application\msedge.exe).VersionInfo.ProductVersion
$NewEdge = (Get-EdgeEnterpriseMSI -Channel Stable -VersionCheck)
if (!(Get-ChildItem -Path $WinTemp | Where-Object {$_.Name -like "MicrosoftEdgeEnterprise*"})) {
  if ($NewEdge -ne $CurrentEdge) {
    Get-EdgeEnterpriseMSI -Channel Stable -Folder $WinTemp
  }
  else {
    Write-Host "Skipping Edge download..."
  }
}
else {
  Write-Host "Skipping Edge download..."
...
if ($NewEdge -ne $CurrentEdge) {
  ...
  Start-Process -FilePath $FinalDest -ArgumentList "/qn"
}
else {
  Write-Host "Skipping Edge install"
}

Set-ItemProperty -Path HKLM:\Software\Policies\Microsoft\Edge -Name HideFirstRunExperience -Value 1 -Force -ErrorAction SilentlyContinue
