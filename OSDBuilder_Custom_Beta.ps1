...
$SuccessArray = @()
try {
    Import-Module PSWriteHTML -ErrorAction Stop | Out-Null
}
catch {
    Install-Module PSWriteHTML -Force -SkipPublisherCheck | Out-Null
    Import-Module PSWriteHTML | Out-Null
}
...
$Paths = [PSCustomObject]@{
    ...
    OSDBPath = ($Env:SystemDrive + "\" + "Build")
...
if ($Paths.OSDBModule -ne "") {
    try {
        Write-Host "Removing existing OSDBuilder PoSH module..." -ForegroundColor DarkMagenta 
        #Uninstall-Module -AllVersions -Name OSDBuilder -ErrorAction Stop | Out-Null
        $SuccessArray += (Write-Output "$($Paths.OSDBModule) uninstalled" | Write-SuccessArray)
    }
    catch {
        $ErrorArray += (Write-Output $_.Exception)
    }
...
try {
    Write-Host "Installing OSDBuilder Module..." -ForegroundColor Green
    $Modules = @("OSD","OSDBuilder","OSDSUS")
    $Modules | ForEach {
        $Module = $_
        try {
            ...
            Install-Module -Name $Module | Out-Null
        }
        finally {
            Update-Module -Name $Module -Force | Out-Null
        }
    }
    Update-OSDSUS
...
try { 
    ...
    Copy-Item -Path "$($Paths.WIMServicing)\OSDBuilder\Content\CMTrace.exe" -Destination ($Paths.OSDBPath + "\Content\ExtraFiles\CMTrace\Windows\System32") -Force -ErrorAction Stop | Out-Null
    $SuccessArray += (Write-Output "Copied file to path..." | Write-SuccessArray)
    Copy-Item -Path "$($Paths.WIMServicing)\OSDBuilder\scripts\PSWindowsUpdate" -Destination ($Paths.OSDBPath + "\Content\ExtraFiles\scripts\Windows\Installer") -Recurse -Force -ErrorAction Stop | Out-Null
    $SuccessArray += (Write-Output "Copied file to path..." | Write-SuccessArray)
...
if (Get-Module -Name WindowsImageTools) {
    ...
    $vhdPath = "$WIM2VHD\virtual disks"
    $vmPath = "$WIM2VHD\virtual machines"

    Convert-Wim2VHD -Path "$vhdPath\$vhd.vhdx" -SourcePath $WIM -Index 1 -Size 40GB -DiskLayout UEFI -Dynamic
